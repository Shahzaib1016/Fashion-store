...
export const TheShoes = ({isReactClassroom, loggedIn}) => {
  console.log('shoes list rendered');
  const studentList = isReactClassroom ? Jordans : Jordans;
  return (
    <div className="student-list">
      <Table class="primary">
        ...
        <tbody>
          {studentList.map(({id, name, price, description, stocks}, index) => (
            <tr key={index}>
              <td>{id}</td>
              <td>{name}</td>
              <td>{price}</td>
